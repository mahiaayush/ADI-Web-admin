...
import { getTestimonial, deleteTestimonial } from "src/store/actions/TestimonialAction";
...
export const TestimonialCorrection = () => {
  ...
  const handleDelete = async () => {
    ...
    const { data, error } = await dispatch(deleteTestimonial(id));
    let message = data?.message;
    if (error) {
      msgType = 'error';
      message = error?.message;
    }
    dispatch(getTestimonial(page + 1, limit)).then(() => setIsLoading(false));
  ...
  const testimonialData = useSelector(
    (state: any) => state?.getTestimonialReducer?.testimonialResponse?.data
  )
  
  const ActionHandler = (row: TestimonialObject) => {
      ...
      {confirmDialog && (
        ...
        {isMessage && (
          ...
          <Card>
            ...
            >
              { testimonialData?.rows?.length > 0 && (
                <div className={`${classes.tfoottop} itemListSorting`}>
                  <EnhancedTable
                    isLoading={isLoading}
                    columns={columns}
                    data={testimonialData?.rows}
                    totalCount={testimonialData?.count}
                    limit={limit}
                    currentPage={page}
                    setPage={setPage}
                    setLimit={setLimit}
