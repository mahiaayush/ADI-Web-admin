...
export default function AddCourseContentPopup({ openImport, setOpenImport, SeccountId = 0, pageNo = 1, limit = 10 }) {
  ...
  useEffect(() => {
    if (SeccountId > 0) {
      const rowData = allCourseSectionData?.rows?.find((itm) => itm?.SeccountId === SeccountId);
  ...
  const handleSubmit = (e) => {
    e.preventDefault()
    console.log("isEditable ==>", SeccountId);
    if (SeccountId > 0) {
      // Update Dispatcher
      dispatch(updateCourseSection(SeccountId, courseSectionData)).then((data: any) => {
        if (data?.isSuccess) {
    ...
    <div id="dsdds">
      <Dialog open={openImport} fullWidth={true} maxWidth="md">
        <div style={{ position: 'relative', width: '100%', padding: '10px 15px' }}>
          ...
          <h2 style={{ textAlign: 'center' }}>{(SeccountId === 0) ? 'Add' : 'Edit'} Course-Section </h2>
          <DialogContent>
