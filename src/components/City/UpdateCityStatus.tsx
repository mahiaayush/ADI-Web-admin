...
export default function UpdateCityStatus({
  updateStatusDialog,
  ...
  useEffect(() => {
    if (updateStatusDialog) {
      setCityStatus(existingCityStatus === "A" ? "I" : "A");
    }
  }, [updateStatusDialog]);
  const handleAdd = async () => {
    ...
    let res;
    try {
      res = await http.patch(`${ADMIN_API_ENDPOINT_V2}${CITY_DATA}`, cityObj);
    ...
    <>
      ...
      >
        <DialogContent style={{ width: "500px" }}>
          ...
          <p>
            {existingCityStatus === "A"
              ? "Do you want to Deactivate City?"
              : "Do you want to Activate City?"}
          </p>
        </DialogContent>
        <DialogActions>
